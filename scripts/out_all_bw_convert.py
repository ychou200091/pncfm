...
import sys
sys.path.append("/home/admin123/Desktop/project/grad/env/codeM/algorithms/")
from flow_info import * 
#from "home/admin123/Desktop/project/grad/env/codeM/algorithms/" import flow_info
...
# ]
input_files = [
    "{}_{}.out".format(src[1:], dst[1:])
    for (src, dst) in iperf_flow_info.keys()
]
# file_list = {
#     "1_3.csv": {"name": "Flow 1_3", "start_time":  iperf_flow_info[("h1","h3")]["start_time"], "flow_set":("h1","h3")},
#     "2_4.csv": {"name": "Flow 2_4", "start_time":  iperf_flow_info[("h2","h4")]["start_time"], "flow_set":("h2","h4")},
#     "9_10.csv": {"name": "Flow 9_10", "start_time":  iperf_flow_info[("h9","h10")]["start_time"], "flow_set":("h9","h10")},
#     "5_6.csv": {"name": "Flow 5_6", "start_time":  iperf_flow_info[("h5","h6")]["start_time"], "flow_set":("h5","h6")}
# } 


file_list = {
    "{}_{}.csv".format(src[1:], dst[1:]): {
        "name": "Flow {}_{}".format(src[1:], dst[1:]),
        "start_time": flow_info["start_time"],
        "flow_set": (src, dst)
    }
    for (src, dst), flow_info in iperf_flow_info.items()
}
...
def generate_profit_loss_csv(df):
    results = {}
    ...
    for flow in df['Flow'].unique():
        ...
        for file in file_list.values():
            ...
            if file["name"] == flow:
                target_flow = file["flow_set"]
        p = iperf_flow_info[target_flow]["profit"]
        
        flow_data = df[df['Flow'] == flow]
        packets = flow_data['Total_Packets'].sum()
        lost = flow_data['Packet_Lost'].sum()
        loss_rate = lost / packets if packets else 0
        results[f"{flow}'s Profit Gain"] = round( p*(1-loss_rate),4)
        results[f"{flow}'s Profit Lost"] = p*(loss_rate)
        results[f"{flow}'s Profit Lost Rate"] = loss_rate

        total_profit = total_profit+p
        total_profit_lost = total_profit_lost + p*(loss_rate)
        

    total_loss_rate = total_profit_lost / total_profit if total_profit else 0
    results["Network's Total Profit(Should have gain)"] = total_profit
    results["Network's Total Profit Gain"] = total_profit - total_profit_lost
    results["Network's Profit Lost"] = total_profit_lost
    results["Network's Profit Lost Rate"] = total_loss_rate

    pd.DataFrame([results]).to_csv(os.path.join(file_root, 'profit_loss_summary.csv'), index=False)
...
def generate_line_graphs(df):
    ...
    '''
    ========
    csv: graph_bandwidth_over_time graph_throughput_over_time
    ========
    '''

...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def _detector(self):
        ...
        while CONF.weight == 'delay':
            ...
            try:
                #self.awareness.shortest_paths[:][:]['delay'] = []
                #print "reset delay list"
                #print self.awareness.shortest_paths[1]
                self.logger.debug("Refresh the shortest_paths")
            except:
                #print "reset delay list fell"
                self.awareness = lookup_service_brick('awareness')
        ...
        try:
            for src in self.awareness.graph:
                for dst in self.awareness.graph[src]:
                    ...
                    if (src ==1 and dst ==3)  or (src ==3 and dst ==1):
                        self.awareness.graph[src][dst]['delay'] = 5
        except:
            if self.awareness is None:
                self.awareness = lookup_service_brick('awareness')
            return

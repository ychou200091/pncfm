...
class NetworkAwareness(app_manager.RyuApp):
    ...
    def get_graph(self, link_list):
        ...
        for src in self.switches:
            for dst in self.switches:
                if src == dst:
                    self.graph.add_edge(src, dst, weight=0,end=dst)

		
                elif (src, dst) in link_list:
		    if src == 13 or src ==14 or src ==15 or src ==16 or src ==17 or src ==18 or dst == 13 or dst ==14 or dst ==15 or dst ==16 or dst ==17 or dst ==18 :
			self.graph.add_edge(src, dst, weight=6,end=dst)

		    if (src ==2 and dst==4) or (src ==4 and dst==2) :
			self.graph.add_edge(src, dst, weight=10,end=dst)
		    
                    else:
			self.graph.add_edge(src, dst, weight=1,end=dst)
        ...
        if self.pre_access_table != self.access_table:
            ...
            if not self.access_table.keys():
                ...
                for tup in self.access_table:
                    print tup,":", self.access_table[tup]
    ...
    def update_in_out_sw(self, flow, in_sw, out_sw): # 7,14
        ...
        print "update_in_out_sw"
        for ip,mac in self.access_table:
            if ip == flow[0]: # in-switch update
                
                self.access_table[(ip,mac)] = (in_sw,out_door[in_sw])
                print  "(ip,mac)",(ip,mac) , "(in_sw,out_door[in_sw]):",(in_sw,out_door[in_sw])
                # self.access_table[(ip,mac)][1] = 
            if ip == flow[1]: # out-switch update
                self.access_table[(ip,mac)] = (out_sw,out_door[out_sw])
                print  "(ip,mac)",(ip,mac) , "(in_sw,out_door[out_sw]):",(in_sw,out_door[out_sw])
                # self.access_table[(ip,mac)][1] = 

...
class Controller_Communication(app_manager.RyuApp):
        ...
        if jsondata['command'] =='Help' :
            ...
            if gateway_domain[gateway_link[jsondata['in_switch']]] == switch_domain[CONF.ofp_tcp_listen_port] and gateway_domain[gateway_link[jsondata['out_switch']]] == switch_domain[CONF.ofp_tcp_listen_port]:
                ...
                if path :
                    ...
                    # update in_switch of flow.
                    # key: (src_ip,dst_ip), 
                    # values[list]: 0: gateway-gateway sw into current domain. 
                    # 1:  gateway-gateway sw back to original domain. 
                    # 2: path,    #3: adjustment count, #4: slice # 5: org_bw, 6: org_profit
    ...
    def send_help(self,in_switch,out_switch,src_ip,dis_ip,max_bw,max_token,s):
        ...
        if((src_ip,dis_ip) not in self.help_list.keys()):
            self.help_list.setdefault((src_ip,dis_ip),[25,time()])
        else:
    ...
    def help_reply_reroute(self, flow, in_switch ,out_switch ):
        """
        ...
        out_port = link_to_port[(path[0],path[1])][0]
        ...
        print " link_to_port[(path[0],path[1])]: ",  link_to_port[(path[0],path[1])]
